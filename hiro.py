...
def create_en_agents(params, device, n_en):
    ...
    for _ in range(n_en):
        ...
        gate_net = Gate(goal_dim+goal_dim, 1).to(device)
...
def train(params):
    ...
    for t in range(step, max_timestep):
        ...
        if t < start_timestep:
            ...
            a_tmp, mask = en_utils.en_pick_action(state, goal, en_agents, max_action, (t+1)%c==1, t, goal_dim, epsilon=1.0, ucb_lamda=1., gates=gates, option='e-greedy',gate_pretrain_steps=t-start_timestep)     # episode_timestep_h==1      (t+1)%c==1
